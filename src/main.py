...
def train_player(board_size, train_set_size):
    ...
    X, Y = game_executor.prepare_train_set(board_size=board_size, n_games=train_set_size)
...
def main():
    ...
    winners1 = game_executor.play(board_size=3, starting_player=super_player, n_games=100, with_ui=False)
    print("Swap")
    game_executor = GameExecutor(random, super_player)
    winners2 = game_executor.play(board_size=3, n_games=100, with_ui=False)
    print("Total stats:")
